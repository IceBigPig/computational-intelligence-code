...
global local_tabu_list, local_tabu_time, city_num, city, tabu_list, tabu_time, tabu_limit, city_i, best_s, best_distance


def init():
    global local_tabu_list, local_tabu_time, city_num, city, tabu_list, tabu_time, tabu_limit, city_i, \
        best_s, best_distance
    # 城市数量
    city_num = 4
    # 城市坐标位置和距离
    city = [
...
# 清空临时交换表
def init_temp_arr():
    global local_tabu_list, local_tabu_time
    local_tabu_time = []
    local_tabu_list = []
...
# 开始进行迭代
def start_iteration():
    global tabu_list, tabu_time, city_i, best_distance, best_s
    for t in range(0, 10):
        # 临时变量
        local_i = -1
        local_j = -1
        ...
        if local_best_fit < best_distance:
            ...
            best_s = city_i
        print("第", t + 1, "代", best_distance)
    print("最优解：", best_distance, best_s)
...
if __name__ == "__main__":
    ...
    for i in range(0, 6):
        ...
        init_temp_arr()
        start_iteration()
        table.cell(row=i+1, column=1).value = best_distance
        # 保存文件
    f.save('TS_2_res.xlsx')
